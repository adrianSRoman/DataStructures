...
int BinarySearchTreeX<Comparable>::height(BinaryNodeX<Comparable> *t) const
{
  ...
  //else
  //  return rightHeight;
  return rightHeight;  // to be written by student
}
...
int BinarySearchTreeX<Comparable>::sum(BinaryNodeX<Comparable> *t) const
{
  if (!t)
    return 0;
  return t->element + sum(t->right) + sum(t->left); // to be written by student
}
...
{
  BinaryNodeX<Comparable> *ancestor = find(possibleAncestor, root);
  BinaryNodeX<Comparable> *max = findMax(ancestor);
  BinaryNodeX<Comparable> *min = findMin(ancestor);

  if ((x >= min->element && x <= ancestor->element) 
      || (x > ancestor->element && x <= max->element))
    return true;
...
int BinarySearchTreeX<Comparable>::highestFull(BinaryNodeX<Comparable> *t) const
{
  ...
  int leftHeight = highestFull(t->left);
  int rightHeight = highestFull(t->right);  
...
int BinarySearchTreeX<Comparable>::deepestFull(BinaryNodeX<Comparable> *t) const
{
  ...
  int leftHeight = deepestFull(t->left);
  int rightHeight = deepestFull(t->right);
